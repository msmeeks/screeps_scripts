	...
	apply: function() {
		...
		Creep.prototype.gatherEnergy = function(targetType) {
			...
			if (!targetType || targetType == FIND_STRUCTURES) {
				var storedTarget = this.pos.findClosestByRange(FIND_STRUCTURES, {filter: x => (x.structureType == STRUCTURE_CONTAINER || x.structureType == STRUCTURE_STORAGE) && x.store[RESOURCE_ENERGY] > 200});
